...
COMMON_DARTS = ['Miss']
BULL_DARTS = ['25','Bull']
CRICKET_ALLOWED_DARTS = COMMON_DARTS + [f"{letter}{number}" for number in range(15, 21) for letter in ['S', 'D', 'T']] + BULL_DARTS
X01_ALLOWED_DARTS = COMMON_DARTS + [f"{letter}{number}" for number in range(1, 21) for letter in ['S', 'D', 'T']] + BULL_DARTS
...
class DartSelect(AutoDartChildEntity,SelectEntity):
    ...
    def options(self) :
        ...
        if (self.idx <= actual_throw and not self.coordinator.data.finished) or (self.idx < actual_throw and self.coordinator.data.finished) :
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        await self.coordinator.data.async_throw(self.get_segment_from_name(option),throw_id=throw_id)
