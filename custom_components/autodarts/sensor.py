...
from .entity import AutoDartChildEntity, AutoDartEntity, to_translation_key
...
async def async_setup_entry(
    ...
    async_add_entities([match_sensor, turn_sensor, board_state_sensor] + player_sensors)
...
class BoardStateSensor(AutoDartEntity, SensorEntity):
    ...
    def native_value(self) -> str | None:
        ...
        if self.coordinator.data:
            return to_translation_key(self.coordinator.data.state["status"])
...
class PlayerSensor(AutoDartChildEntity, SensorEntity):
    ...
    def segments(self):
        if self.is_playing:
            return {
                number: value[self.idx]
                for number, value in self.coordinator.data.state["segments"].items()
            }

    @property
    def winner(self):
        if self.is_playing and self.coordinator.data.finished:
...
class MatchSensor(AutoDartChildEntity, SensorEntity):
    ...
    options = [MATCH_WAITING, MATCH_STARTED, MATCH_STOPPED]
