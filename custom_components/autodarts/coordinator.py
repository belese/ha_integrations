...
from autodarts import CloudBoard, Match, Lobby
...
class AutoDartsBoardMatchCoordinator(AutoDartsGenericMatchCoordinator) :
    ...
    def wait(self) :
        ...
        handler_cb.append(
            self.board_coordinator.item.register_async_callback(on_board_reset,"Manual reset")
        )
    ...
    def load(self, item, forward_state = True) :
        ...
        self._unregister_cb.append( self.item.register_async_callback(async_on_match_ended,event="delete",topic="events") )
        self._unregister_cb.append( self.item.register_async_callback(async_on_match_ended,event="finish",topic="events") )
        self.connect()
    ...
    async def _async_update_data(self) :
        if not self.board_coordinator.item.match_id:
            ...
            match = await Match.from_id(self.session,self.board_coordinator.item.match_id)
